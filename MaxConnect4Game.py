...
class maxConnect4Game:
    ...
    def AI_Play(self):
        ...
        self.currentTurn = self.ch_PlyrTurn(self.currentTurn)


    def ch_PlyrTurn(self, player):
        if player == 2:
            return 1
        elif player == 1:
            return 2
    ...
    def computer_Turn(self, column, turn, minMaxGameBord):
    ...
    def minMax(self, minmaxPlayer, alpha, beta, depthLevel, column, minMaxGameBord, player):
        ...
        if minmaxPlayer:
            ...
            for i in range(0,7):
                result, row = self.computer_Turn(i, player, minMaxGameBord)
                if result:
                    minMaxGameBord = result.copy()
                    utility, col = self.minMax(False, alpha, beta, depthLevel+1, i, minMaxGameBord, self.ch_PlyrTurn(player))
            ...
            for i in range(0,7):
                ...
                if result:
                    minMaxGameBord = result.copy()
                    utility, col = self.minMax(True, alpha, beta, depthLevel+1, i, minMaxGameBord, self.ch_PlyrTurn(player))
                    minMaxGameBord[row][i] = 0
